...
namespace Serialization
{
    class Atom
    {
        ...
        {
            string[] split = objectdata.Split();

            int proton = Convert.ToInt32(split[1]);
            int neutron = Convert.ToInt32(split[2]);
            double weight = Convert.ToDouble(split[3]);

            Atom element = new Atom(split[0], proton, neutron, weight, split[4]);
